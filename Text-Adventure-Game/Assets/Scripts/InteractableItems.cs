...
public class InteractableItems : MonoBehaviour
{
    ...
    public void AddActionResponsesToUseDictionary()
    {
        for (int i = 0; i < nounsInInventory.Count; i++)
        {
            ...
            InteractableObject interactableObjectInInventory = GetInteractableObjectFromUsableİtemsList(noun);
    ...
    InteractableObject GetInteractableObjectFromUsableİtemsList(string noun)
    {
        for (int i = 0; i < usableItemList.Count; i++)
        {
            if (noun == usableItemList[i].noun)
            {
